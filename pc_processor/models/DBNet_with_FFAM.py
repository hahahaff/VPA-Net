...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        layer3_out = self.dropout(self.layer3(layer2_out))
        layer4_out = self.dropout(self.layer4(layer3_out))
...
class Fusin_Attention(nn.Module):
    def __init__(self, channel, ratio=4):
        super(Fusin_Attention, self).__init__()
        self.conv1_spatial = nn.Sequential(
            nn.Conv2d(channel, 1, kernel_size=1, bias=True),
            nn.Sigmoid()
        )
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.conv1_channel = nn.Conv2d(channel, channel, 1, bias=True)

    def forward(self, x):
        spatial_fusemap = self.conv1_spatial(x) * x

        feat_vec = self.avg_pool(spatial_fusemap)  # b,c,1,1
        feat_vec = nn.Softmax(dim=1)(self.conv1_channel(feat_vec)) * feat_vec.shape[1]  # b,c,1,1

        channel_fusemap = feat_vec * spatial_fusemap # b,c,h,w

        return channel_fusemap
...
class FFAM(nn.Module):
    def __init__(self, pcd_channels, img_channels):
        ...
        self.pcd_attention = Fusin_Attention(pcd_channels)
    ...
    def forward(self, pcd_feature, img_feature):
        ...
        att = self.pcd_attention(fuse_out)
        out = att + pcd_feature

        return out

class SalsaNextwithFFAM(SalsaNext):
    def __init__(self, in_channels=8, nclasses=20, base_channels=32, img_feature_channels=[]):
        super(SalsaNextwithFFAM, self).__init__(in_channels=in_channels, base_channels=base_channels, nclasses=nclasses, softmax=True)

        self.fusionblock_1 = FFAM(self.base_channels * 2,img_feature_channels[0])
        self.fusionblock_2 = FFAM(self.base_channels * 4,img_feature_channels[1])
        self.fusionblock_3 = FFAM(self.base_channels * 8,img_feature_channels[2])
        self.fusionblock_4 = FFAM(self.base_channels * 8,img_feature_channels[3])
        self.aspp = ASPP(self.base_channels * 8, self.base_channels * 8)  # 256,256
...
class DBNet(nn.Module):
    def __init__(self, pcd_channels=5, img_channels=3, nclasses=20, base_channels=32, 
        ...
        self.camera_branch_encoder = ResNet(
            in_channels=img_channels, 
            pretrained=imagenet_pretrained,
            backbone=image_backbone)
        
        self.camera_branch_decoder = RGBDecoder(
            self.camera_branch_encoder.feature_channels, 
            nclasses=nclasses, base_channels=self.camera_branch_encoder.expansion*16)

        self.lidar_branch = SalsaNextwithFFAM(
            in_channels=pcd_channels, nclasses=nclasses, base_channels=base_channels,
            img_feature_channels=self.camera_branch_encoder.feature_channels)
    
    def forward(self, pcd_feature, img_feature): 
        img_feature = self.camera_branch_encoder(img_feature) 
        lidar_pred, lidar_last_feat = self.lidar_branch(pcd_feature, img_feature) 
        camera_pred = self.camera_branch_decoder(img_feature) 
        
        return lidar_pred, lidar_last_feat, camera_pred
