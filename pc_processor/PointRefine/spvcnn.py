...
class SPVCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x0 = self.stem.to('cuda')(x0)
        z0 = voxel_to_point(x0, z, nearest=False)
        z0.F = z0.F

        x1 = point_to_voxel(x0, z0)
        x1 = self.stage1.to('cuda')(x1)
        x2 = self.stage2.to('cuda')(x1)
        x3 = self.stage3.to('cuda')(x2)
        x4 = self.stage4.to('cuda')(x3)
        z1 = voxel_to_point(x4, z0)
        z1.F = z1.F + self.point_transforms[0].to('cuda')(z0.F)

        y1 = point_to_voxel(x4, z1)
        y1.F = self.dropout(y1.F)
        y1 = self.up1[0].to('cuda')(y1)
        y1 = torchsparse.cat([y1, x3])
        y1 = self.up1[1].to('cuda')(y1)

        y2 = self.up2[0].to('cuda')(y1)
        y2 = torchsparse.cat([y2, x2])
        y2 = self.up2[1].to('cuda')(y2)
        z2 = voxel_to_point(y2, z1)
        z2.F = z2.F + self.point_transforms[1].to('cuda')(z1.F)

        y3 = point_to_voxel(y2, z2)
        y3.F = self.dropout(y3.F)
        y3 = self.up3[0].to('cuda')(y3)
        y3 = torchsparse.cat([y3, x1])
        y3 = self.up3[1].to('cuda')(y3)

        y4 = self.up4[0].to('cuda')(y3)
        y4 = torchsparse.cat([y4, x0])
        y4 = self.up4[1].to('cuda')(y4)
        z3 = voxel_to_point(y4, z2)
        z3.F = z3.F + self.point_transforms[2].to('cuda')(z2.F)

        out = self.classifier.to('cuda')(z3.F)
