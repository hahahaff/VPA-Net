...
class SemanticKitti(object):
    def __init__(self, root,
        ...
        for k, v in sem_color_inv_map.items():
            if k + 1 > max_sem_key:
                max_sem_key = k + 1 # max_sem_key=20
    ...
    def mapLidar2Camera(self, seq, pointcloud, img_w, img_h):
        ...
        mapped_points = (proj_matrx @ pointcloud_hcoord.T).T
        mapped_points = mapped_points[:, :2] / \
                        np.expand_dims(mapped_points[:, 2], axis=1)
        keep_idx_pts = (mapped_points[:, 0] > 0) * (mapped_points[:, 0] < img_w) * (
                mapped_points[:, 1] > 0) * (mapped_points[:, 1] < img_h)
...
class SemanticKittiRGB(SemanticKitti):
    def __init__(self, root,
        ...
        for seq in self.sequences:
            ...
            pointcloud_path = os.path.join(self.root, seq, "eight_channel_data")
            pointcloud_files = [os.path.join(pointcloud_path, f) for f in os.listdir(
                pointcloud_path) if ".bin" in f]
